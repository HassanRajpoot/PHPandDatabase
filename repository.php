...
$sql = "SELECT stock_id, stock_quantity, stock_item,stock_type, stock_purchase_price, stock_sale_price FROM stock";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>" . $row["stock_id"]. "</td><td>" . $row["stock_item"] . "</td><td>" . $row["stock_quantity"] . "</td><td>" . $row["stock_type"] . "</td><td>" . $row["stock_purchase_price"] . "</td><td>"
. $row["stock_sale_price"]. "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
?>
          ...
          <div class="flex pl-4 mt-4 lg:w-2/3 w-full mx-auto">
            ...
            <button onclick="document.location='insertrepos.php'"class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">ADD ITEM</button>
            <button onclick="document.location='deleterepos.php'"class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">DELETE ITEM</button>
          </div>
        </div>
      </section>
